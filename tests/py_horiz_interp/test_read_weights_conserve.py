...
def write_remap():
    ...
    tile1_cell = np.array(
        [[i, j] for j in range(1, ny_src + 1) for i in range(1, nx_src + 1)] * 4,
        dtype=np.int32,
    )

    tile2_cell = [[i, j] for j in range(1, ny_src + 1) for i in range(1, nx_src + 1)]
    tile2_cell += [
        [i + nx_src, j] for j in range(1, ny_src + 1) for i in range(1, nx_src + 1)
    ]
    tile2_cell += [
        [i, j + ny_src] for j in range(1, ny_src + 1) for i in range(1, nx_src + 1)
    ]
    tile2_cell += [
        [i + nx_src, j + ny_src]
        for j in range(1, ny_src + 1)
        for i in range(1, nx_src + 1)
    ]
...
def test_read_weights_conserve():
    ...
    interp_id = pyfms.horiz_interp.read_weights_conserve(
        "remap.nc", "fregrid", nx_src, ny_src, domain, src_tile=pyfms.mpp.pe() + 1
    )

    data_src = np.array(
        [[pe * 1000 + j * 100 + i for i in range(nx_src)] for j in range(ny_src)],
        dtype=np.float64,
    )
