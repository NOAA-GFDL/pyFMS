...
def test_vector_update_domains():
    ...
    cyclic_global_domain = 2 #check
    fold_north_edge = 2**5 #check
    ...
    for i in range(nx):
        for j in range(ny):
            global_data1[i+whalo][j+shalo] = 1 + (i+whalo)*1e-3 + (j+shalo)*1e-6
            global_data2[i+whalo][j+shalo] = 1 + (i+whalo)*1e-3 + (j+shalo)*1e-6
    ...
    for i in range(nx+ehalo+1):
        for j in range(nhalo):
            ...
            global_data1[nx+whalo+1][j+ny+shalo] = -global_data1[nx-1][ny+1-j]
    ...
    if mpp.pe(): #will change after merge
        assert np.array_equal(x_data, global_data1[:xsize_d][:ysize_d])
